...
class ExchangeClient:
    def __init__(self):
        ...
        self.exchange = ccxt.binance({
            ...
            'options': {
                'defaultType': 'spot',
                'fetchMarkets': {'spot': True, 'margin': False, 'swap': False, 'future': False},
    ...
    def _verify_credentials(self):
        ...
        if not self.settings.BINANCE_API_KEY or not self.settings.BINANCE_API_SECRET:
            error_msg = "配置错误: 缺少 BINANCE_API_KEY 或 BINANCE_API_SECRET"
        ...
        try:
            ...
            for i in range(max_retries):
                try:
                    ...
                    self.logger.info(f"市场数据加载成功 | 交易对: {self.settings.SYMBOL}")
                    return True
