...
from services.sentiment_service.app.core.config import PUSHPLUS_TOKEN, PUSHPLUS_TIMEOUT
import time
import psutil
import os
...
def send_pushplus_message(content, title="交易信号通知"):
    """【修改】修改此函数以从 settings 对象获取 PUSHPLUS_TOKEN。"""
    
    # 【新增】在函数内部获取配置实例
    settings = get_settings()
    
    # 【修改】从 settings.PUSHPLUS_TOKEN 获取令牌
    if not settings.PUSHPLUS_TOKEN:
        logging.error("未配置PUSHPLUS_TOKEN，无法发送通知") # 这条日志可以保留，也可以在配置加载时警告
        return
    
    url = os.getenv('PUSHPLUS_URL', 'https://www.pushplus.plus/send')
    data = {
        "token": settings.PUSHPLUS_TOKEN, #【修改】
        "title": title,
        "content": content,
        "template": "txt"
    }
