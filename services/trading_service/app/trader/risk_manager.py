        ...
        try:
            ...
            if abs(position_ratio - self.last_position_ratio) > 0.001:
                self.logger.info(
                    ...
                    f"最大允许比例: {self.trader.settings.MAX_POSITION_RATIO:.2%} | "
                    f"最小底仓比例: {self.trader.settings.MIN_POSITION_RATIO:.2%}"
                )
                self.last_position_ratio = position_ratio
            
            if position_ratio < self.trader.settings.MIN_POSITION_RATIO:
                self.logger.warning(f"底仓保护触发 | 当前: {position_ratio:.2%}")
                return True
            
            if position_ratio > self.trader.settings.MAX_POSITION_RATIO:
    ...
    async def check_market_sentiment(self):
        ...
        try:
            ...
            if fear_greed < 20:  # 极度恐惧
                self.trader.settings.RISK_FACTOR *= 0.5  # 降低风险系数
            elif fear_greed > 80:  # 极度贪婪
                self.trader.settings.RISK_FACTOR *= 1.2  # 提高风险系数
        except Exception as e:
