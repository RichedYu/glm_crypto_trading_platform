...
from app.core.api_proxy_pool import ApiProxyPool
from app.trader.order_tracker import OrderTracker, OrderThrottler
from app.trader.risk_manager import AdvancedRiskManager
import logging
import asyncio
import numpy as np
...
import time
import math
from app.core.helpers import send_pushplus_message, format_trade_message
import json
#from app.trader.monitor import TradingMonitor# Monitor的逻辑可以整合或保持独立
from app.trader.position_controller_s1 import PositionControllerS1
...
class GridTrader:
    def __init__(self, exchange: ExchangeClient):
        """
        交易器初始化。
        不再接收config对象，而是内部调用 get_settings()。
        """
        self.settings = get_settings()
        self.exchange = exchange
        self.logger = logging.getLogger(self.__class__.__name__)
        self.symbol = self.settings.SYMBOL
        self.base_price = self.settings.INITIAL_BASE_PRICE
        self.grid_size = self.settings.INITIAL_GRID
        ...
        self.price_history = []
        self.last_grid_adjust_time = 0 # 初始化为0，确保第一次能执行
        self.symbol_info = None
        self.last_volatility = 0.0 # 用于传递给策略服务
        ...
        proxy_timeout = max(1.0, self.settings.API_TIMEOUT / 1000)
        self.strategy_api_pool = ApiProxyPool(
            service_name="strategy_service",
            endpoints=self.settings.STRATEGY_SERVICE_ENDPOINTS,
            timeout=proxy_timeout,
            failure_threshold=self.settings.API_PROXY_FAIL_THRESHOLD,
            cooldown=self.settings.API_PROXY_COOLDOWN_SECONDS
        )

    
    async def initialize(self):
        """【保留】初始化逻辑基本不变，仅修改配置的引用方式"""
        if self.initialized:
            return
        ...
        try:
            ...
            # 【修改】使用 self.settings 获取 SYMBOL
            self.symbol_info = self.exchange.exchange.market(self.settings.SYMBOL)

            # 【修改】使用 self.settings 获取 INITIAL_BASE_PRICE
            if self.settings.INITIAL_BASE_PRICE > 0:
                ...
                if self.base_price is None:
                     raise ValueError("无法获取初始实时价格")
                self.logger.info(f"使用实时基准价: {self.base_price}")

            # 【保留】后续逻辑不变
            await self._check_and_transfer_initial_funds()
            self.logger.info(f"初始化完成 | 交易对: {self.settings.SYMBOL} | 基准价: {self.base_price}")
            send_pushplus_message(
                f"交易服务启动成功\n交易对: {self.settings.SYMBOL}\n基准价: {self.base_price:.2f} USDT",
                "系统通知"
            )
            ...
            send_pushplus_message(f"交易服务启动失败\n错误: {e}", "错误通知")
            raise
    ...
    async def run_one_cycle(self) -> dict:
        ...
        【最终版核心循环】
        1. 首先，尝试进行一次智能参数调整。
        2. 然后，使用最新的参数执行常规的交易检查和操作。
        """
        try:
            if not self.initialized:
                await self.initialize()

            # 1. 【思考与决策】调用大脑，动态调整策略参数
            await self.adjust_parameters_dynamically()
            
            # 2. 【感知】获取当前市场价格
            self.current_price = await self._get_latest_price()
            if not self.current_price:
                return {"status": "skipped", "reason": "无法获取最新价格"}

            self.logger.info(f"--- 使用当前网格 {self.grid_size:.2f}% | 基准价 {self.base_price:.2f} | 现价 {self.current_price:.2f} ---")
            
            # 3. 【行动】执行交易逻辑
            # S1 策略优先
            if await self.position_controller_s1.check_and_execute():
                return {"status": "action_taken", "source": "s1_strategy"}

            # 主网格策略
            if await self._check_sell_signal():
                return await self.execute_order('sell')
            
            if await self._check_buy_signal():
                return await self.execute_order('buy')

            # 风控检查
            await self.risk_manager.multi_layer_check()
            
            return {"status": "no_signal", "current_price": self.current_price, "grid_size": self.grid_size}

        except Exception as e:
            self.logger.error(f"交易周期发生严重错误: {e}", exc_info=True)
            return {"status": "cycle_failed", "error": str(e)}

    async def adjust_parameters_dynamically(self):
        """
        核心智能调度器：收集情报 -> 请求大脑决策 -> 应用决策
        """
        # 每小时最多调整一次，避免过于频繁
        if time.time() - self.last_grid_adjust_time < 3600:
            return

        self.logger.info("="*20 + " 开始智能决策周期 " + "="*20)
        
        # 1. 收集情报：获取最新的市场波动率
        await self._update_latest_volatility()
        
        # 2. 请求大脑决策
        params = await self._get_dynamic_parameters_from_strategy_service()
        
        # 3. 应用决策
        if params and 'recommended_grid_size' in params:
            new_grid_size = params['recommended_grid_size']
            if abs(new_grid_size - self.grid_size) > 0.1:
                self.logger.warning(f"决策执行：网格大小调整！ 旧: {self.grid_size:.2f}%, 新: {new_grid_size:.2f}%")
                self.grid_size = new_grid_size
                send_pushplus_message(
                    f"🧠 网格智能调整\n预测波动率: {params['predicted_volatility']:.3f}\n推荐网格: {new_grid_size:.2f}%",
                    "策略更新"
                )
        
        self.last_grid_adjust_time = time.time()
        self.logger.info("="*20 + " 智能决策周期结束 " + "="*20)

    async def _update_latest_volatility(self):
        """获取并更新最新的市场波动率"""
        try:
            klines = await self.exchange.fetch_ohlcv(self.settings.SYMBOL, '1h', self.settings.VOLATILITY_WINDOW)
            if len(klines) < self.settings.VOLATILITY_WINDOW:
                self.logger.warning("K线数据不足，无法计算精确波动率")
                return
            ...
            self.last_volatility = float(volatility) if pd.notna(volatility) else self.last_volatility
            self.logger.info(f"情报获取：最新年化波动率为 {self.last_volatility:.4f}")
        except Exception as e:
            self.logger.error(f"更新波动率失败: {e}", exc_info=True)

    async def _get_dynamic_parameters_from_strategy_service(self):
        """调用策略服务API，获取智能参数"""
        if self.last_volatility <= 0:
            self.logger.warning("当前波动率数据无效，无法请求策略服务。")
            return None

        try:
            input_data = {
                # 在这个版本中，我们简化了，让大脑自己去拿情感数据
                # 未来可以传递更多信息给大脑
                "sentiment_score_lag1": 0, 
                "volatility_lag1": self.last_volatility
            }
            
            self.logger.info("正在通过策略服务代理池请求决策...")
            strategy_params = await self.strategy_api_pool.request_json(
                method="POST",
                path="/api/v1/predict/dynamic-parameters",
                json=input_data
            )
            self.logger.info(f"大脑决策：接收到新参数: {strategy_params}")
            return strategy_params
        except Exception as e:
            health = self.strategy_api_pool.health_snapshot()
            self.logger.error(
                f"无法通过策略服务代理池获取策略参数: {e} | 健康状态: {health}"
            )
            return None
    ...
    def _get_flip_threshold(self):
        ...
        【修正】
        计算价格反转阈值。
        不再使用全局lambda，而是从settings中读取因子。
        """
        return (self.grid_size / 100) * self.settings.FLIP_THRESHOLD_FACTOR
    ...
    async def _sync_recent_trades(self, limit: int = 50):
        ...
        """
        try:
            latest_fills = await self.exchange.fetch_my_trades(self.settings.SYMBOL, limit=limit)
            if not latest_fills:
    ...
    async def _check_buy_signal(self):
        if not self.current_price: return False
        lower_band = self._get_lower_band()
        if self.current_price <= lower_band:
            self.buying_or_selling = True
            new_lowest = self.current_price if self.lowest is None else min(self.lowest, self.current_price)
            if new_lowest != self.lowest:
                self.lowest = new_lowest
                self.logger.info(f"买入监测 | 当前价: {self.current_price:.2f} | 触发价: {lower_band:.2f} | 最低价: {self.lowest:.2f}")
            ...
            if self.lowest and self.current_price >= self.lowest * (1 + threshold):
                ...
                self.logger.info(f"触发买入信号 | 当前价: {self.current_price:.2f} | 已从最低价 {self.lowest:.2f} 反弹")
                return await self.check_buy_balance(self.current_price)
        else:
            if self.buying_or_selling: # 如果之前在监测，现在退出了
    ...
    async def _check_sell_signal(self):
        ...
        if self.current_price >= upper_band:
            self.buying_or_selling = True
            new_highest = self.current_price if self.highest is None else max(self.highest, self.current_price)
            if new_highest != self.highest:
                self.highest = new_highest
                self.logger.info(f"卖出监测 | 当前价: {self.current_price:.2f} | 触发价: {upper_band:.2f} | 最高价: {self.highest:.2f}")

            threshold = self._get_flip_threshold()
            if self.highest and self.current_price <= self.highest * (1 - threshold):
                self.buying_or_selling = False
                self.logger.info(f"触发卖出信号 | 当前价: {self.current_price:.2f} | 已从最高价 {self.highest:.2f} 回调")
                return await self.check_sell_balance()
        else:
    ...
    async def get_available_balance(self, currency):
        ...
        # 【修正】使用 SAFETY_MARGIN
        return balance.get('free', {}).get(currency, 0) * self.settings.SAFETY_MARGIN
    ...
    async def _calculate_dynamic_interval_seconds(self):
        ...
        try:
            ...
            interval_rules = self.settings.DYNAMIC_INTERVAL_PARAMS['volatility_to_interval_hours']
            default_interval_hours = self.settings.DYNAMIC_INTERVAL_PARAMS['default_interval_hours']
            ...
            default_interval_hours = self.settings.DYNAMIC_INTERVAL_PARAMS.get('default_interval_hours', 1.0)
    ...
    async def _ensure_trading_funds(self):
        ...
        try:
            ...
            # 计算所需资金
            required_usdt = self.settings.MIN_TRADE_AMOUNT * 2  # 保持两倍最小交易额
    ...
    async def execute_order(self, side):
        """执行订单，带重试机制"""
        max_retries = self.settings.MAX_RETRIES  # 最大重试次数
        retry_count = 0
        ...
        while retry_count < max_retries:
            try:
                # 获取最新订单簿数据
                order_book = await self.exchange.fetch_order_book(self.settings.SYMBOL, limit=5)
                ...
                # 检查订单状态
                updated_order = await self.exchange.fetch_order(order_id, self.settings.SYMBOL)
                ...
                try:
                    await self.exchange.cancel_order(order_id, self.settings.SYMBOL)
                    self.logger.info(f"订单已取消，准备重试 | ID: {order_id}")
                except Exception as e:
                    # 如果取消订单时出错，检查是否已成交
                    self.logger.warning(f"取消订单时出错: {str(e)}，再次检查订单状态")
                    try:
                        check_order = await self.exchange.fetch_order(order_id, self.settings.SYMBOL)
...
"""
    ...
    async def _adjust_grid_after_trade(self):
        ...
        if trade_count % self.settings.GRID_PARAMS.get('adjust_interval', 5) == 0: # 增加默认值以防万一
            volatility = await self._calculate_volatility()

            # 根据波动率调整
            if volatility > self.settings.GRID_PARAMS['volatility_threshold']['high']:
                new_size = min(
                    self.grid_size * 1.1,  # 扩大10%
                    self.settings.GRID_PARAMS['max']
                )
                action = "扩大"
            else:
                new_size = max(
                    self.grid_size * 0.9,  # 缩小10%
                    self.settings.GRID_PARAMS['min']
                )
                action = "缩小"
        ...
        for order_id, timestamp in list(self.order_timestamps.items()):
            if current_time - timestamp > self.ORDER_TIMEOUT:
                try:
                    ...
                    order = await self.exchange.fetch_order(order_id, self.settings.SYMBOL, params)
                    ...
                    if order['status'] == 'closed':
                        ...
                        await self.exchange.cancel_order(order_id, self.settings.SYMBOL, params)
    ...
    async def adjust_grid_size(self):
        ...
        try:
            ...
            # 根据波动率获取基础网格大小
            base_grid = self.settings.INITIAL_GRID
            for range_config in self.settings.GRID_PARAMS['volatility_threshold']['ranges']:
            ...
            # 确保网格在允许范围内
            new_grid = max(min(new_grid, self.settings.GRID_PARAMS['max']), self.settings.GRID_PARAMS['min'])
    ...
    async def _calculate_volatility(self):
        ...
        try:
            ...
            klines = await self.exchange.fetch_ohlcv(
                self.settings.SYMBOL,
                timeframe='1h',
                limit=self.settings.VOLATILITY_WINDOW
            )
    ...
    def _adjust_amount_precision(self, amount):
        if not self.symbol_info: return amount
        precision = self.symbol_info.get('precision', {}).get('amount', 8)
        factor = 10 ** precision
    ...
    async def calculate_trade_amount(self, side, order_price):
        ...
        risk_adjusted_amount = min(
            total_assets * self.settings.RISK_FACTOR * volatility_factor * kelly_f * percentile_factor,
            total_assets * self.settings.MAX_POSITION_RATIO
        )

        # 应用最小/最大限制
        amount_usdt = max(
            min(risk_adjusted_amount, self.settings.BASE_AMOUNT),
    ...
    async def _get_order_price(self, side):
        """获取订单价格"""
        try:
            order_book = await self.exchange.fetch_order_book(self.settings.SYMBOL)
    ...
    async def _get_price_percentile(self, period='7d'):
        ...
        try:
            # 获取过去7天价格数据（使用4小时K线）
            ohlcv = await self.exchange.fetch_ohlcv(self.settings.SYMBOL, '4h', limit=42)  # 42根4小时K线 ≈ 7天
    ...
    async def _calculate_required_funds(self, side):
        ...
        return min(required, self.settings.MAX_POSITION_RATIO * total_assets)
    ...
    async def _check_flip_signal(self):
        ...
        try:
            ...
            flip_threshold = self.base_price * self._get_flip_threshold()
    ...
    def _calculate_dynamic_base(self, total_assets):
        ...
        # 计算基于总资产百分比的交易金额范围
        min_amount = max(
            self.settings.MIN_TRADE_AMOUNT,  # 不低于20 USDT
            total_assets * self.settings.MIN_POSITION_PERCENT  # 不低于总资产的5%
        )
        max_amount = total_assets * self.settings.MAX_POSITION_PERCENT  # 不超过总资产的15%
